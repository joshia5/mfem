...
{
   apf::MeshEntity* ent;
   apf::MeshIterator* itr = o_mesh->begin(BcDim);

   while ((ent=o_mesh->iterate(itr)))
   {
      apf::ModelEntity* mdEnt = o_mesh->toModel(ent);
      if (o_mesh->getModelType(mdEnt) == BcDim)
      {
         NumBc++;
      }
   }
...
void Omega_hMesh::Load(Omega_h::Mesh* o_mesh, int generate_edges, int refine,
                    bool fix_orientation)
{
  ...
  //
   ...
   if (curved && read_gf)
   {
      ...
      Nodes = new GridFunctionPumi(this, o_mesh, v_num_loc,
                                   crd_shape->getOrder());
      edge_vertex = NULL;
      own_nodes = 1;
...
//
   ...
   while ((ent = o_mesh->iterate(itr)))
   {
      // Get vertices
      apf::Downward verts;
      o_mesh->getDownward(ent,0,verts); // num_vert
...
//
   ...
   int BCdim = Dim - 1;
   NumOfBdrElements = 0;
   CountBoundaryEntity(o_mesh, BCdim, NumOfBdrElements);
   boundary.SetSize(NumOfBdrElements);
   j=0;

   // Read boundary from SCOREC mesh
   itr = o_mesh->begin(BCdim);
   while ((ent = o_mesh->iterate(itr)))
   {
      ...
      {
         apf::Downward verts;
         o_mesh->getDownward(ent, 0, verts);
         int attr = 1;
...
// ParOmega_hMesh implementation
// This function loads a parallel PUMI mesh and returns the parallel MFEM mesh
// corresponding to it.
ParOmega_hMesh::ParOmega_hMesh(MPI_Comm comm, apf::Mesh2* o_mesh,
                         int refine, bool fix_orientation)
{
   ...
   apf::Numbering* vLocNum =
      apf::numberOwnedDimension(o_mesh, "AuxVertexNumbering", 0);
   ...
   Array<Pair<long,int> > thisVertIds(o_mesh->count(0));
   itr = o_mesh->begin(0);
   for (int i = 0; (ent = o_mesh->iterate(itr)); i++)
   {
   ...
   v_num_loc = o_mesh->findNumbering("LocalVertexNumbering");
   if (!v_num_loc)
      v_num_loc = apf::createNumbering(o_mesh,
   ...
   // Fill the elements
   NumOfElements = countOwned(o_mesh,Dim);
   elements.SetSize(NumOfElements);

   // Read elements from SCOREC Mesh
   itr = o_mesh->begin(Dim);
   for (int j = 0; (ent = o_mesh->iterate(itr)); j++)
   {
      // Get vertices
      apf::Downward verts;
      o_mesh->getDownward(ent,0,verts);
   ...
   // Read boundary from SCOREC mesh
   itr = o_mesh->begin(BcDim);
   for (int bdr_ctr = 0; (ent = o_mesh->iterate(itr)); )
   {
      // Check if this mesh entity is on the model boundary
      apf::ModelEntity* mdEnt = o_mesh->toModel(ent);
      if (o_mesh->getModelType(mdEnt) == BcDim)
   ...
   if (Dim > 2)
   {
      ...
      apf::Numbering* AuxFaceNum =
         apf::numberOwnedDimension(o_mesh, "AuxFaceNumbering", 2);
   ...
   if (Dim > 1)
   {
      ...
      apf::Numbering* AuxEdgeNum =
         apf::numberOwnedDimension(o_mesh, "EdgeNumbering", 1);
   ...
   for (int i = 0; i < sfaces.Size(); i++)
   {
      apf::Mesh::Type ftype = o_mesh->getType(sfaces[i].two);
   ...
   // Build shared_trias and shared_quads. They are allocated above.
   {
      ...
      for (int i = 0; i < sfaces.Size(); i++)
      {
         ...
         int *v, nv = 0;
         apf::Mesh::Type ftype = o_mesh->getType(ent);
         if (ftype == apf::Mesh::TRIANGLE)
         {
   ...
   {
      GridFunctionPumi auxNodes(this, o_mesh, v_num_loc,
                                crd_shape->getOrder());
...
void ParOmega_hMesh::UpdateMesh(const ParMesh* AdaptedpMesh)
{
...
int ParOmega_hMesh::RotationPUMItoMFEM(apf::Mesh2* o_mesh,
                                    apf::MeshEntity* tet,
                                    int elemId)
{
   MFEM_ASSERT(o_mesh->getType(tet) == apf::Mesh::TET, "");
   // get downward vertices of PUMI element
   apf::Downward vs;
   int nv = o_mesh->getDownward(tet,0,vs);
   int pumi_vid[12];
   for (int i = 0; i < nv; i++)
   {
   ...
   return ma::findTetRotation(o_mesh, tet, vs_rot);
}

// Convert parent coordinate form a PUMI tet to an MFEM tet
IntegrationRule ParOmega_hMesh::ParentXisPUMItoMFEM(apf::Mesh2* o_mesh,
                                                 apf::MeshEntity* tet,
                                                 int elemId,
                                                 ...
                                                 bool checkOrientation)
{
   ...
   int rotation = checkOrientation ? RotationPUMItoMFEM(o_mesh, tet, elemId):0;
...
void ParOmega_hMesh::ParentXisMFEMtoPUMI(apf::Mesh2* o_mesh,
                                      int elemId,
                                      apf::MeshEntity* tet,
...
void ParOmega_hMesh::FieldMFEMtoPUMI(apf::Mesh2* o_mesh,
                                  ...
                                  apf::Field* vel_mag_field)
{
   ...
   int dim = o_mesh->getDimension();

   apf::MeshEntity* ent;
   apf::MeshIterator* itr = o_mesh->begin(dim);
   int iel = 0;
   while ((ent = o_mesh->iterate(itr)))
   {
      apf::NewArray<apf::Vector3> pumi_nodes;
      apf::getElementNodeXis(field_shape, o_mesh, ent, pumi_nodes);
      IntegrationRule mfem_nodes = ParentXisPUMItoMFEM(
                                      o_mesh, ent, iel, pumi_nodes, true);
      // Get the solution
      ...
      for (int d = 0; d <= dim; d++)
      {
         ...
         apf::Downward a;
         int na = o_mesh->getDownward(ent,d,a);
         for (int i = 0; i < na; i++)
         {
            int type = o_mesh->getType(a[i]);
...
// adaptation
void ParOmega_hMesh::VectorFieldMFEMtoPUMI(apf::Mesh2* o_mesh,
                                        ParGridFunction* grid_vel,
                                        apf::Field* vel_field,
                                        apf::Field* vel_mag_field)
{
...
void ParOmega_hMesh::NedelecFieldMFEMtoPUMI(apf::Mesh2* o_mesh,
                                         ParGridFunction* gf,
                                         apf::Field* nedelec_field)
{
   ...
   size_t elemNo = 0;
   apf::MeshEntity* ent;
   apf::MeshIterator* it = o_mesh->begin(dim);
   while ( (ent = o_mesh->iterate(it)) )
   {
      // get all the pumi nodes and rotate them
      apf::NewArray<apf::Vector3> pumi_nodes;
      apf::getElementNodeXis(nedelecFieldShape, o_mesh, ent, pumi_nodes);
      IntegrationRule mfem_nodes = ParentXisPUMItoMFEM(
                                      o_mesh, ent, elemNo, pumi_nodes, true);
      ...
      for (int d = 0; d <= dim; d++)
      {
         ...
         for (int i = 0; i < na; i++)
         {
            ...
            apf::MeshElement* me = apf::createMeshElement(o_mesh, a[i]);
            for (int n = 0; n < nan; n++)
            {
   ...
   o_mesh->end(it); // end loop over all elements
}

void ParOmega_hMesh::FieldPUMItoMFEM(apf::Mesh2* o_mesh,
                                  apf::Field* field,
                                  ParGridFunction* grid)
{
   ...
   for (int i = 0; i < pmesh->GetNE(); i++)
   {
      ...
      apf::MeshEntity* ent = o_mesh->iterate(it);
      apf::NewArray<apf::Vector3> pumi_xi(non);
      ParentXisMFEMtoPUMI(o_mesh,
                          i,
                          ent,
                          mfem_xi,
                          pumi_xi,
      ...
      apf::MeshElement* me = apf::createMeshElement(o_mesh, ent);
